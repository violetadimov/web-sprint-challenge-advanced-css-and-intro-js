...
/* Task 2: There is a typo in your dataset 😱 The 9th artist, Vincent Van Gogh
 is currently Vincent Van Dough. Use an array method to fix this issue 
 and console.log() to check your work. */

function artistName(array, index, artist) {
    array[index].name = artist;
    return array;
}
console.log(artistName(artists, 8, "Vincent Van Dough"));
...
and returns an array with names of artists who lived in the 20th century 
(1900-2000) */
...
function get20s(artists) {
    ...
    for (let i = 0; i < artists.length; i++) {
        if (artists[i].years.includes("18")) {
...
console.log(addArtist(artists, 20, "Violeta", "1993 - current day", "Web Design", "American", 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse iaculis sapien in molestie blandit. Vestibulum mattis libero eget augue vehicula, ac maximus eros aliquet. Ut consectetur eros ut mi auctor aliquam.'))
...
/* STRETCH 2: Create a function called `randomize` that takes a data array
 as an argument and returns a the same array in a randomized order. */
...
your MVP code (create an array of all artists born in the 1900s with .filter, 
  for example) */
let filteredArray = artists.filter(function(artist1900) {
    if (artists.includes("19")) {
        return artist1900;
    }
})
